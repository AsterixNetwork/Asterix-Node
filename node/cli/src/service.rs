...
use asterix_node_runtime::RuntimeApi;
use sc_service::{
...
mod tests {
	...
	use asterix_node_runtime::{BalancesCall, Call, UncheckedExtrinsic, Address};
	use asterix_node_runtime::constants::{currency::CENTS, time::SLOT_DURATION};
	use codec::Encode;
	use sp_core::{
	...
	#[test]
	// It is "ignored", but the asterix-cli ignored tests are running on the CI.
	// This can be run locally with `cargo test --release -p asterix-cli test_sync -- --ignored`.
	#[ignore]
	fn test_sync() {
